...
Transient left-hand report: after acceptance, the user reported the left hand looked wrong while the right was correct. Read-only code review identified possible pose-dependent mask initialization and elbow-estimate behavior, but no definite cause was established. Burst captures showed both hands, and the user subsequently said “الان درسته”. No code or APK was changed during this investigation; this is not recorded as a confirmed bug fix.
